...
class MITREMapper:
    TECHNIQUES = {
        "T1190": {"name": "SQL Injection", "patterns": ["' OR", "UNION SELECT", "--", "1=1"]},
...
class OllamaTriage:
    def analyze(self, log_entry, mitre_techs):
        prompt = f'Return ONLY valid JSON: {{"risk": "low/medium/high/critical", "action": "block/monitor/ignore", "summary": "one sentence"}}\nLog: {log_entry}\nMITRE: {mitre_techs}'
        try:
...
def classify_risk(request, status_code, user_agent, ip, timestamp_str, failure_count=0, args=None):
    mitre_matches = MITREMapper().map_request(request, status_code, user_agent, ip, timestamp_str, failure_count)
    result = {"ip": ip, "request": request, "status_code": status_code, "mitre_techniques": mitre_matches}
    if args and getattr(args, 'ollama', False):
        result["ai_triage"] = OllamaTriage().analyze(request, mitre_matches)
    return result
